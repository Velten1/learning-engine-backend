...
import { startPomodoroService, abandonmentPomodoroService, getCurrentPomodoroService, completePomodoroService, resetPomodoroService } from "../services/pomodoroService"
...
export const completePomodoro = async (req: Request, res: Response) => {
    try {
        ...
        if (!pomodoro) {
            return res.status(404).json({ message: 'Nenhum pomodoro ativo encontrado' })
        }
        return res.status(200).json(pomodoro)
    } catch (error: any) {
...
//pickup userID and send to service to reset pomodoro session
//controller dont need prisma cliente, it will be used in the repository
//repository will be used in the service

export const resetPomodoro = async (req: Request, res: Response) => {
    try {
        const userId = (req as any).userId as string
        const pomodoro = await resetPomodoroService(userId as string)
        return res.status(200).json(pomodoro)
    } catch (error: any) {
        console.error('Erro ao resetar o pomodoro:', error.message)
        const statusCode = error.statusCode || 500
        const message = error.message || 'Erro interno do servidor'
        return res.status(statusCode).json({ message: message })
    }
